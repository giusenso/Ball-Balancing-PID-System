 ...
 */
void drawObjectV2(bool ball_detected, Point_t ball,  Mat &frame, bool noise_error){
	...
	if (ball_detected){	//////////////////////////////////////
		...
		rectangle(	frame,
					cv::Point(ball.x-28, ball.y-28),
					cv::Point(ball.x+28, ball.y+28),
					GREEN, 1, LINE_8, 0);
		
		//display ball info
		putText(frame,intToString(ball.y),cv::Point(ball.x+2,ball.y-42),1,1,BLUE,2);
		line(frame, cv::Point(ball.x, 0), cv::Point(ball.x, FRAME_HEIGHT), BLUE, 1);
		line(frame, cv::Point(0, ball.y), cv::Point(FRAME_WIDTH, ball.y), BLUE, 1);
		putText(frame,intToString(ball.y),cv::Point(ball.x+2,ball.y-42),1,1,BLUE,2);
		putText(frame,intToString(ball.x),cv::Point(ball.x+40,ball.y+14),1,1,BLUE,2);
        ...
        if(numObjects<MAX_NUM_OBJECTS){
			for (int index = 0; index >= 0; index = hierarchy[index][0]) {
                ...
                if(area>MIN_OBJECT_AREA && area<MAX_OBJECT_AREA && area>refArea){
					ball->x = (moment.m10/area)+(FRAME_WIDTH-CONTROL_AREA)/2; 
					ball->y = (moment.m01/area)+(FRAME_HEIGHT-CONTROL_AREA)/2;
					refArea = area;
					*ball_detected = true;
				}
			}
